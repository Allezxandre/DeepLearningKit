...
extension  UIImage {
    ...
    */
    public func setPixelColorAtPoint(_ point:CGPoint, color: RawColorType) -> UIImage? {
    ...
    func applyOnPixels(_ closure:ModifyPixelsClosure) -> UIImage? {
        ...
        for x in 0 ..< Int(pixelsWide)  {
            for y in 0 ..< Int(pixelsHigh)  {
                ...
                let (newRedColor, newGreenColor, newBlueColor, newAlphaValue)  =  closure(point: CGPoint(x: CGFloat(x), y: CGFloat(y)), redColor: red!, greenColor: green!,  blueColor: blue!, alphaValue: alpha!)
